...
/*pte_t* swap_policy(pde_t *page_dir){
  pte_t *pte;
  for (int addr =0;addr<= KERNBASE ;addr+= PGSIZE){
    pte = walkpgdir(page_dir,(void *)addr,0);
    if (pte != 0){
      if (*pte & PTE_P){
        if (!(*pte & PTE_A)){
          return pte;
...
pte_t* swap_policy(pde_t *page_dir){
    ...
    {
          if((pte = walkpgdir(page_dir, (char*)i, 0)) != 0)
      {
        if(*pte==0)
          return (pte_t*)(pte);
        if(*pte & PTE_P)
...
pte_t *swap_page_out(pde_t *page_dir){
  ...
  if (*pte == 0){
    clearaccessbit(page_dir);
    pte = swap_policy(page_dir);
  }
  ...
  for (int i =0; i<8;i++){
    begin_op();
    buffer = bread(ROOTDEV, b_page+i);
    memmove(buffer->data, page_buffer + i*512, 512);
    log_write(buffer);
    brelse(buffer);
    end_op();
  }
...
void page_fault_handler(){
  ...
  addr &= ~0xfff;
  pde_t *page_dir = current_process -> pgdir;
  pte_t *pte;
  pte = walkpgdir(page_dir, (void*)addr, 1);


  cprintf("Inital pte = %d \n",*pte);
  if ((*pte & PTE_PS) == 128 && (*pte & PTE_P) == 0){
    cprintf("Swapping In!\n");
    //cprintf("Pte = %d\n",*pte);
    
    int blk = getswappedblk(page_dir, addr);
    cprintf("Block-ID : %d\n", blk);
    char buffer[PGSIZE]="";
